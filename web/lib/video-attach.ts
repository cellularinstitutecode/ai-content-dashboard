...
export async function attachPendingVideos(args: {
  ...
  if (norm.degraded || !norm.media.length) {
    ...
    const failure = normalizeFailure({ status: norm.failure?.status ?? null, error: norm.failure?.error ?? null, shape: norm.failure?.shape ?? null, attempts: norm.failure?.attempts ?? null });
