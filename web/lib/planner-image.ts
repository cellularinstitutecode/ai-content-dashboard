...
export const CRAFT = [
  ...
  'NOTHING ATTACHED TO ANYONE: no medical device is worn by, strapped to, clipped to or connected to a person, and none is held as though it ',
  'were being given. No blood-pressure cuff on an arm, no ECG leads or electrodes, no pulse oximeter on a finger, no IV line, drip, cannula or ',
  'port, no glucose monitor or patch, no syringe, injector pen, vial or ampoule in anyone\'s hand, no oxygen mask or cannula, no bandage or ',
  'dressing. A device may rest ON THE TABLE, unused and not connected to anybody. This is a conversation, never a procedure.',
