...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (r.ok) {
      const post = (resp && resp.data) ? resp.data : resp;
      ...
      return NextResponse.json({ ok: true, id, status, publicationDate: post && post.publicationDate, providers: post && post.providers });
    }
